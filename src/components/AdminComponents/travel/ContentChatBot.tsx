  ...
  const handleSave = async () => {
    ...
    await Promise.all(
      questions.map(q => {
        ...
        return setDoc(doc(db, getCollectionName('travelchatbotQuestions', selectedLanguage), q.id), dataToSave);
      })
    );
  ...
  return (
    <div className={styles.adminChatbotBox}>
            ...
            <div key={qIdx} className={styles.adminChatbotItem}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center',  borderRadius: 8, padding: 8, marginBottom: 4 }}>
                <input
                  ...
                  style={{ width: 140, fontWeight: 600, color: '#d7b76e' }}
              ...
              <div style={{ marginLeft: 12, marginTop: 6, background: '#fcfcfc', borderRadius: 6, padding: 8 }}>
                  ...
                  {q.options && q.options.length > 0 ? (
                    q.options.map((opt, oIdx) => (
                      <li key={oIdx} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 2 }}>
                        ...
                        />
                        <button className={styles.adminChatbotBtn}  style={{backgroundColor:'#d7b76e'}} onClick={() => handleRemoveOption(qIdx, oIdx)} title="Seçeneği Sil">Sil</button>
                      </li>
                    ))
                  ) : (
                    <li>Seçenek yok.</li>
                ...
                <button className={styles.adminChatbotBtn}  style={{backgroundColor:'#d7b76e', marginTop: 4}} onClick={() => handleAddOption(qIdx)}>Seçenek Ekle</button>
                ...
                {q.redirect ? (
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 4 }}>
                    ...
                    <input
                      ...
                      style={{ flex: 1 }}
                    />
                    <button className={styles.adminChatbotBtn} style={{backgroundColor:'#d7b76e' }} onClick={() => handleRemoveRedirect(qIdx)} title="Yönlendirmeyi Sil">Sil</button>
                  </div>
                ) : (
                  <button className={styles.adminChatbotBtn} onClick={() => handleRedirectChange(qIdx, 'type', 'url')} style={{ marginTop: 4,backgroundColor:'#d7b76e' }}>Yönlendirme Ekle</button>
                )}
      ...
      <div style={{ display: 'flex', marginTop: 16 }}>
        ...
        <button className={styles.adminChatbotBtn} style={{ backgroundColor:'#d7b76e' }} onClick={handleSave}>Kaydet</button>
      </div>
